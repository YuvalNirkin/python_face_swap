...
class FaceModel:
    def __init__(self, path, name):
        ...
        self.eyemask = self.getEyeMask(width=80,plot=False)
        self.mouthmask = self.getMouthMask(width=80,plot=False)
        self.nosemask = self.getNoseMask(width=80,plot=False)
    ...
    def getMouthMask(self,width=1, plot=False):
        X = self.ref_U[:,:,0]
        X = X.reshape( (-1), order='F' )
        Y = self.ref_U[:,:,1]
        Y = Y.reshape( (-1), order='F' ) 
        ...
        [idxs, dist] = sklearn.metrics.pairwise_distances_argmin_min(self.model_TD, cloud)
        mouth = idxs[48:67]
        output = self.createMask(mouth, width=width)
        if plot:
            plt.figure()
            plt.imshow(output)
            plt.draw()
    ...
    def getNoseMask(self,width=1, plot=False):
        X = self.ref_U[:,:,0]
        X = X.reshape( (-1), order='F' )
        Y = self.ref_U[:,:,1]
        Y = Y.reshape( (-1), order='F' ) 
    ...
    def createMask(self,eyeLeft,width=1):
        ...
        if '3.'  in cv2.__version__:
            contours = cv2.findContours(eyemask,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
        else:
        ...
        if '3.' not in cv2.__version__: #this does not work with opencv3
            for r in range(self.ref_U.shape[0]):
                for c in range(self.ref_U.shape[1]):
                    if cv2.pointPolygonTest(contours[0], (c,r), False ) > 0:
                        eyemaskfill[r,c,:] = 255
        else:
            cv2.drawContours(eyemaskfill, contours[1], -1, (255,255,255), -1)
