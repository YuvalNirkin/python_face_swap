...
def demo():
    ...
    proj_matrix1, camera_matrix, rmat, tvec = calib.estimate_camera(allModels[allModels.keys()[0]], lmarks1[0])
    cap = cv2.VideoCapture(sys.argv[2])
    writer = cv2.VideoWriter('output.avi',cv.CV_FOURCC('X','V','I','D'),20,(img1.shape[1],img1.shape[0]))
    while(cap.isOpened()):
        ret, img2 = cap.read()
        if ret == False:
            continue
        ...
        if len(lmarks2) != 0:
            ...
            for poseId in listPose:
                ...
                for subj in range(1,nSub+1):
					...
					print '> Looking at file: ', sys.argv[1]
					print '> Looking at file: ', sys.argv[2]
					...
					img1a  = renderer.swap(img1.copy(), img2.copy(), proj_matrix1, proj_matrix2,\
